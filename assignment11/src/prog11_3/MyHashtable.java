...
public class MyHashtable implements Iterable {
	...
	private void rehash() {
		int preTableSize = tableSize;
		tableSize = tableSize * 2;
		LinkedList<Entry>[] newTable = new LinkedList[tableSize];
		 
		for(int i = 0; i < preTableSize; ++i) {
			if(table[i] != null) {
	...
	public static void main(String[] args) {
		...
		System.out.println("old tableSize: " + h.tableSize);
		for(int i = 0; i < 100; ++i) {
			h.put(i,i);
		} 
		
		System.out.println("numEntries: " + h.numEntries);	 
		System.out.println("New tableSize: " + h.tableSize);
		System.out.println("Num entries: " + h.numEntries);
