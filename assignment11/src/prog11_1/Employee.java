package prog11_1;

import java.util.Arrays;
import java.util.HashMap;
import java.util.Iterator;

public class Employee {
	private String firstName;
	private String lastName;
	private HashMap<String, Double> salaryRecord;
	
	Employee(){
		salaryRecord = new HashMap<String, Double> ();
	}
	
	public void addEntry(String date, double salary) 
	{ 
		salaryRecord.put(date, salary);
	}
	
	public void printPaymentAmount(String date) 
	{
		 Object ob = salaryRecord.get(date);
		 
		 String s =  this.getFirstName() +" "+ this.getLastName() ;
		 if(ob == null) 
		 {
			 s += " did not receive a paycheck on " +date;
		 }
		 else 
		 { 
			 s +=  " was paid " + ob + " on " +date; 
		 } 
		 System.out.println(s); 
	}
	
	public void printAveragePaycheck() {
		
		double total = 0;
		double count =0;
		for(double salary : salaryRecord.values()) { 
			total += salary; 
			count++; 
		}
		
		double avg = total / count;
		 
		String s = "Average paycheck for " + this.getFirstName() + " "+ this.getLastName()+ " was " + avg;
		
		System.out.println(s);  
	}
	
	public static void main(String[] args) {
		Employee e = new Employee();
		e.setFirstName("Jim");
		e.setLastName("Jones");
		for(int i = 1; i <= 12; ++i) {
			e.addEntry(i+"/15/2011", 3070+5*i);
		}
		e.printPaymentAmount("3/15/2011");
		e.printPaymentAmount("5/15/2010");
		e.printAveragePaycheck(); 
		 
	}
	
	public String getFirstName() {
		return firstName;
	}
	
	public void setFirstName(String firstName) {
		this.firstName = firstName;
	}
	
	public String getLastName() {
		return lastName;
	}
	
	public void setLastName(String lastName) {
		this.lastName = lastName;
	}

}
